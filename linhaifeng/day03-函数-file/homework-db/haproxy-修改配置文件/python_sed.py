...
#[{'backend':'www.oldboy.com','record':{'server':'1.1.1.11','weight':22,'maxconn':33}},{'backend':'www.oldboy.com','record':{'server':'1.1.1.11','weight':22,'maxconn':33}}]
###写盘
...
def fetch(data):
    ...
    #file_handle('haproxy.conf',backend_data)
    return file_handle('haproxy.conf',backend_data)
###添加
def add(data):
...
def change(data):
    ...
    if not record_list or old_current_record not in record_list:
        print('\033[33;51m 无33此记录\033[0m')
        return
    else:
        record_list.insert(0, backend_data)
